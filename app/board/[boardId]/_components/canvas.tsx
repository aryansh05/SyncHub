...
import { CanvasMode, CanvasState, Camera, Color, LayerType, Point, Side, XYWH } from "@/types/canvas";
...
const unselectLayers = useMutation((
  ...
  const layerIdArray: string[] = liveLayerIds ? [...liveLayerIds] : [];
...
useEffect(() => {
  function onKeyDown(e: KeyboardEvent) {
    ...
    // Don't handle keys if user is typing
    const isTyping = 
    ...
    if (e.ctrlKey || e.metaKey) {
      if (key === "z") {
        e.preventDefault();
        history.undo(); // Ctrl+Z / Cmd+Z
      } else if (key === "y") {
        e.preventDefault();
        history.redo(); // Ctrl+Y / Cmd+Y
      }
    } else if (key === "delete" || key === "backspace") {
      e.preventDefault();
      deleteLayers(); // Delete selected layers
    }
  }
